...
import { BookOpen, Clock, Heart, AlignJustify, Droplets, Star, Trophy } from "lucide-react"
...
export default function AchievementBadges() {
  const { activities, stats, waterGlasses } = useRamadanStore()
  ...
  useEffect(() => {
    ...
    const prayerDays = activities.filter((day) => day.fasting).length
    const dhikrDays = activities.filter((day) => day.dhikrMorning !== "0" || day.dhikrEvening !== "0").length
    const goodDeedsDays = activities.filter(
      (day) => day.charity || day.familyVisit || day.happiness || day.feeding,
    ).length
    ...
    const updatedAchievements: Achievement[] = [
      ...
      },
      {
        id: "hydration",
        title: "مهتم بصحته",
        description: "شرب 8 أكواب من الماء في يوم واحد",
        icon: <Droplets className="h-6 w-6" />,
        color: "blue-500",
        unlocked: waterGlasses >= 8,
        progress: Math.min(waterGlasses, 8),
        target: 8,
      },
      {
        id: "ramadan-half",
  ...
  }, [activities, stats, waterGlasses])

  return (
    <div className="rtl">
      <h2 className="text-2xl font-bold mb-6 text-center">الإنجازات والشارات</h2>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
        {achievements.map((achievement, index) => (
          ...
          >
            <Card className={`${achievement.unlocked ? "border-2 border-purple-300" : "opacity-80"}`}>
              <CardContent className="pt-6 pb-4 px-4">
                <div className="flex flex-col items-center text-center">
                  ...
                  <p className="text-xs text-muted-foreground">
                    {frenchNumerals(achievement.progress)} / {frenchNumerals(achievement.target)}
                    {achievement.unlocked && <span className="text-green-500 mr-2">✓ مكتمل</span>}
                  </p>
                </div>
              </CardContent>
            </Card>
